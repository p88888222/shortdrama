...
// Inisialisasi Hls.js untuk kualitas video M3U8
const hls = new Hls();

// --- 1. ENGINE PLAYER & AUTOHIDE ---
...
function showControls() {
    ...
    // Autohide setelah 3 detik jika video sedang diputar
...
// Event listener untuk interaksi kontrol
...
// --- 2. LOGIKA PENGGABUNGAN VIDEO + SUBTITLE ---
...
function playEp(idx) {
    ...
    // Ambil URL Video dan Subtitle (Gabungan playVoucher + url)
    const videoUrl = ep.playVoucher || ep.videoUrl;
    const subUrl = ep.url || ep.subtitleUrl || ep.m3u8SubtitleUrl;

    // Bersihkan track subtitle lama agar tidak menumpuk
    const tracks = player.querySelectorAll('track');
    tracks.forEach(t => t.remove());

    // Dukungan Kualitas Video M3U8 menggunakan Hls.js
    ...
    // Suntikkan Subtitle dari properti 'url'
    if (subUrl) {
        ...
        track.srclang = "id";
        track.src = subUrl;
        track.default = true; // Langsung aktifkan
        player.appendChild(track);
        
        // Pastikan mode 'showing' setelah track dimuat
    ...
    player.load();
    player.play().catch(() => console.warn("Autoplay block"));
    playIcon.className = "fa-solid fa-pause";
    showControls();

    // Update UI tombol episode
...
// Fitur Auto-Sync Cover yang tidak mengganggu pemuatan data drama
