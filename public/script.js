...
async function switchView(mode, el) {
    // UI Update Navigasi Bawah
    document.querySelectorAll('.nav-item').forEach(b => b.classList.remove('nav-active'));
    if (el) el.classList.add('nav-active');
    
    const content = document.getElementById('appContent');
    content.innerHTML = '<div class="py-20 text-center animate-pulse text-red-600 font-bold text-xs uppercase tracking-[0.3em]">Loading...</div>';
...
function renderContent(data, mode) {
    ...
    categories.forEach(cat => {
        ...
        // Logika Filter: Home tampilkan non-hot, Hot tampilkan kategori hot, ForYou tampilkan semua
...
async function openDetail(id, title, cover, startEp = 1) {
    ...
    const intro = res?.shotIntroduce || "Tidak ada deskripsi tersedia.";
    ...
    if (epData.length > 0) playEpisode((startEp <= epData.length ? startEp - 1 : 0), dramaData);
}
...
function loadLibrary() {
    ...
    content.innerHTML = `
        <div class="space-y-12 pb-20">
            ...
            <section>
                <h2 class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4">Koleksi Saya</h2>
    ...
    else hG.innerHTML = '<p class="text-[10px] text-gray-700 italic py-4">Belum ada riwayat nonton.</p>';
    if (bookmarks.length) bookmarks.forEach(b => bG.appendChild(createDramaCard(b)));
    else bG.innerHTML = '<p class="text-[10px] text-gray-700 italic py-4">Belum ada koleksi.</p>';
}
...
async function performSearch(q) {
    ...
    content.innerHTML = `<h2 class="text-[10px] font-black text-gray-500 mb-6 uppercase tracking-widest">Hasil untuk: ${q}</h2><div id="searchGrid" class="grid grid-cols-3 gap-3 pb-20"></div>`;
    const grid = document.getElementById('searchGrid');
    if (items.length) items.forEach(i => grid.appendChild(createDramaCard(i)));
    else content.innerHTML = '<p class="text-xs text-center opacity-30 py-10">Drama tidak ditemukan.</p>';
}
