async function callApi(path, queryParams = {}) {
    ...
    const response = await fetch(`/api-proxy?path=${encodeURIComponent(path)}&params=${encodeURIComponent(paramsString)}`);
...
async function fetchData(endpoint) {
    ...
    try {
        const result = await callApi(`/netshort/${endpoint}`);
        ...
        return `
            <div class="snap-item w-full">
                ...
                <div class="absolute bottom-32 left-4 right-20 pointer-events-none">
                    <h3 class="text-lg font-bold text-white shadow-black drop-shadow-md">${item.shortPlayName}</h3>
                    <button onclick="showEpisodes('${playId}', '${name}')" class="pointer-events-auto mt-4 bg-red-600 text-white px-6 py-2 rounded-full text-xs font-bold shadow-lg">📂 DAFTAR EPISODE</button>
                </div>
            </div>`;
    }).join('');
}
...
async function showEpisodes(id, title) {
    ...
    try {
        const result = await callApi('/netshort/allepisode', { shortPlayId: id });
        const eps = result.shortPlayEpisodeInfos || [];
        
        list.innerHTML = eps.length === 0 ? `<p class="text-center py-10">Episode Kosong.</p>` : 
        eps.map(ep => `
            <div onclick="playNew('${ep.playVoucher}')" class="flex items-center gap-4 p-4 bg-zinc-800 rounded-xl mb-2">
